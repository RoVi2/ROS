...
void callBack(const geometry_msgs::PointConstPtr & point_ros){
	point_kalman[0] = point_ros->x;
	point_kalman[1] = point_ros->y;
	point_kalman[2] = point_ros->z;
}
...
*/
	...
	Vector3D<double> cameraPos = point_kalman - Vector3D<double>(0.5,0,0);
	//The rotation has to be that which is oriented to the point.
	RPY<double> cameraRotation = RPY<> (angle, 0, 0);
