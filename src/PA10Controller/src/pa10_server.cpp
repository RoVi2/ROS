...
#include <pa10controller/clearJointQueue.h>
...
const uint32_t jointCmdLimitsDEG[] = {170,90,170,140,260,100,360};
...
//void runQueue();
bool getJointConfig_service(pa10controller::getJointConfig::Request  &req, pa10controller::getJointConfig::Response  &res);
bool setJoints_service(pa10controller::setJointConfig::Request  &req, pa10controller::setJointConfig::Response  &res);
bool addtoQueue_service(pa10controller::addToQueue::Request  &req, pa10controller::addToQueue::Response  &res);
bool clearJointQueue_service(pa10controller::clearJointQueue::Request  &req, pa10controller::clearJointQueue::Response  &res);
...
//        raise ValueError('Wrong angles')
 //   return (degree*50*2**14)/360
      ...
      //writeValue(jointCmdAddresses[i], commands[i]);
 ...
 * **************************************************************************************/
{
	...
	if(req.endOfQueue && running) 
	{
		...
		ROS_WARN_COND(!queueSuccess,"Queue failed");
		ROS_DEBUG_COND(queueSuccess, "Queue successfull");
		res.success = queueSuccess;
	}
 ...
 * **************************************************************************************/
{
	...
	//Create item
	queueItem item;
        bool validJoint = true;
	for (uint8_t i = 0; i < 7; i++) 
	{
	 	if(jointCmdLimitsDEG[i] >= fabs(req.positions[i])){	
		
		item.positions[i] = req.positions[i];
      		  	//if(fabs(item.positions[i]) < 0.0005)
            	//item.positions[i] = 0.0005 ;
		item.commands[i] = 5000; //req.commands[i];
    ...
    ROS_INFO("SendSetup command");
	//Send command to controlleri
	if(validJoint)
...
bool startNode_service(rsd_msgs::packML_command::Request &req,rsd_msgs::packML_command::Response &res) {
	...
	addToQueue_client = nodehandle.serviceClient<plc_control::SendPLCCommand>("PLC/SendPLCCommand");
	ROS_INFO("PA10 Server is started.");
	running = true;
	res.message = "";
	res.success = true;
	return true;
...
bool stopNode_service(rsd_msgs::packML_command::Request &req,rsd_msgs::packML_command::Response &res) {
	ROS_DEBUG("Closing services");

	running = false;

	getjointconfigservice.shutdown();
	setjointsservice.shutdown();
	addToQueueService.shutdown();
	//stopnode_service.shutdown();
	//resetnode_service.shutdown();

	ROS_DEBUG("Disconnecting from clients");
	setGripper(false);
	addToQueue_client.shutdown();

	uint8_t head = readTail();

	ROS_DEBUG("Resetting PA10 to default position");
	float resetpositions[7] = {0.01,0.01,0.01,0.01,0.01,0.01,0.01};
	float commands[7] = {5,5,5,5,5,5,5};
	head =  (head + 1 ) % 16;
	sendSetup(head, resetpositions, commands);

	float positions[7] = {0,0,0,0,0,0,0};
	for (uint8_t i = 0; i < 15; i++) {
		head =  (head + 1 ) % 16;
		sendSetup(head, positions, commands);
	}
	...
	//usleep(5.2 * 1000 * 1000);

	disableController();

	ROS_INFO("PA10 Server is stopped.");
	//ros::NodeHandle nodehandle;
	//startnode_service = nodehandle.advertiseService("pa10/start", startNode_service);
...
bool resetNode_service(rsd_msgs::packML_command::Request &req,rsd_msgs::packML_command::Response &res) {
	ROS_DEBUG("PA10 Server is resetting.");
	running = false;
	ROS_INFO("PA10 Server has been reset.");

	ROS_INFO_STREAM("tail: " << readTail());

	if (items_mtx.try_lock())
		items.clear();
	...
	if ((uint32_t)commands[0] == 0x81600000) {
		std::stringstream error;
		error << boost::format("Unable to reset PA10, status code: 0x%08X") % (uint32_t)commands[0];
		res.success = false;
		res.message = error.str();
		ROS_ERROR_STREAM(res.message);
		return true;
